...
export function Waves({
    ...
    const measureTextBounds = () => {
        ...
        const padYTop  = 28   // px above the title
        const padYBot  = 95   // px below — extends to cover the subtitle too
    ...
    const movePoints = (time: number) => {
        ...
        const TEXT_FEATHER = 55   // px — width of the transition region
        const TEXT_PUSH    = 22   // max displacement in px
            ...
            for (let j = 0; j < points.length; j++) {
                ...
                if (tb) {
                    ...
                    if (overlapX > -TEXT_FEATHER && overlapY > -TEXT_FEATHER) {
                        ...
                        if (strength > 0) {
                            // Push toward the nearest edge — Y-axis only so
                            // horizontal lines bend around the text rather than
                            // bunching up on the sides (which would look odd).
                            const pushY = overlapY > 0
                                ? (p.y - (tb.top + tb.bottom) * 0.5 > 0 ? 1 : -1)  // push away from centre
                                : 0
