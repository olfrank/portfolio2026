...
export function Waves({
    ...
    const setLines = () => {
        ...
        for (let i = 0; i < totalLines; i++) {
            ...
            linePositionsRef.current.push(i / Math.max(totalLines - 1, 1))
    ...
    // Map normalised position (0–1) to an HSL colour across indigo → blue → cyan
    const getLineColor = (t: number): string => {
        const h = 260 - t * 75   // 260 (indigo) → 185 (cyan)
        const s = 70 + t * 25    // 70% → 95%
        const l = 50 + t * 18    // 50% → 68%
        return `hsl(${h}, ${s}%, ${l}%)`
    }

    // Draw lines - smooth quadratic bezier curves with colour mapping
    const drawLines = () => {
        const { current: lines } = linesRef
        const { current: paths } = pathsRef
        const { current: positions } = linePositionsRef
        ...
        lines.forEach((points, lIndex) => {
            ...
            // Start at midpoint between first and second so the curve begins smoothly
            const startMidX = (first.x + second.x) / 2
            const startMidY = (first.y + second.y) / 2
            let d = `M ${first.x} ${first.y} L ${startMidX} ${startMidY}`

            // Quadratic bezier through midpoints — current point is the control point,
            // midpoint to next is the on-curve anchor
            ...
            paths[lIndex].setAttribute('stroke', getLineColor(positions[lIndex] ?? 0))
        })
    }
